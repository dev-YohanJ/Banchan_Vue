          ...
          <form @submit.prevent="goDelete">
            <div class="form-group">
              ...
              <input type="password" class="form-control" 
                placeholder="Enter password" v-model.lazy="BOARD_PASS" required>
            </div>
...
export default {
  setup(){
    ...
    const hidden = ()=>{
      BOARD_PASS.value = '' //비밀번호 입력 후 취소를 클릭하면 비밀번호가 남아있어 초기화한다.
      store.dispatch('display', false)
    }
    
    const goDelete = async ()=>{
      const res = await axios.delete(`qna/${num}`,{params:
