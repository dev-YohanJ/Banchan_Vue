	...
	<div class="list">
          ...
          <router-link :to="{name:'Item_Detail', params:{num:`${item.id}`}}">
            <div v-if="item.status == 0" class="picture">
              <img :src="require(`C:/upload/${item.image}`)"/>
            </div>
    ...
    const store = useStore()
        ...
        try{
            ...
            for (let i=0; i < imsi.length; i++) {
                console.log(imsi[i].image)
                imsi[i].image = imsi[i].image.split(',')
                imsi[i].image = imsi[i].image[0]
                console.log("require: "+require(`C:/upload/`+imsi[i].image))
            }
